...
export default function PortfolioPage() {
  ...
  // True High-Fidelity 3D Parallax Layer Rotation Matrix Engine
  const handleCardMouseMove = (e, uniqueId) => {
    ...
    card.style.transform = `perspective(1200px) rotateX(${angleX}deg) rotateY(${angleY}deg) scale3d(1.04, 1.04, 1.04)`;
    
    // Set dynamic shiny laser reflection coordinates
    const shine = card.querySelector(".laser-shimmer");
    if (shine) {
      shine.style.background = `radial-gradient(circle at ${x}px ${y}px, rgba(212,175,55,0.22) 0%, transparent 60%)`;
    }
  };
    ...
    <div style={{ background: "#030406", color: "#f3f4f6", minHeight: "100vh", fontFamily: "'Montserrat', sans-serif", overflowX: "hidden", position: "relative" }}>
      <style>{`
        ...
        *{box-sizing:border-box;}
        ...
        /* Premium elite fluid custom scrollbar */
        ...
        .glow-2 {
          ...
          animation-duration: 55s; animation-delay: -12s; right: -15%; top: 15%;
        }
        .glow-3 {
          background: radial-gradient(circle, rgba(212,175,55,0.03) 0%, transparent 60%);
          animation-duration: 65s; animation-delay: -25s; left: 15%; bottom: -10%;
        }
        ...
        /* Continuous Live Cinema Grain Filter Overlay */
        .film-grain {
        ...
        /* Luxury Cinematic Reveal System animations */
        ...
        .d1 { transition-delay: .15s!important; } .d2 { transition-delay: .35s!important; } .d3 { transition-delay: .55s!important; }

        .serif { font-family: 'Cormorant Garamond', serif; }
        .gold { color: #d4af37; text-shadow: 0 0 30px rgba(212,175,55,0.4); }
        
        /* Ultra Metallic Gold Anisotropic Shimmer Text */
        .gold-grad {
        ...
        /* Elite Interaction Badge Navigators */
        .filter-btn {
          ...
          cursor: pointer; transition: all 0.6s cubic-bezier(0.16, 1, 0.3, 1); text-transform: uppercase;
        ...
        .gallery-item {
          ...
          transition: transform 0.25s cubic-bezier(0.25, 1, 0.5, 1), box-shadow 0.8s cubic-bezier(.16,1,.3,1), border-color 0.8s ease;
        ...
        .gallery-item::before {
          ...
          opacity: 0; transition: opacity 0.7s ease; z-index: 5; pointer-events: none;
        }
        .gallery-item:hover { border-color: transparent; box-shadow: 0 60px 120px rgba(0,0,0,0.9), 0 0 50px rgba(212,175,55,0.12); }
        .gallery-item:hover::before { opacity: 1; }
        
        .gallery-item img { width: 100%; height: 100%; object-fit: cover; filter: brightness(0.65) contrast(1.05) grayscale(15%); transition: transform 0.95s cubic-bezier(.16,1,.3,1), filter 0.95s ease; }
        .gallery-item:hover img { filter: brightness(1.08) contrast(1.1) grayscale(0%); transform: scale(1.15) translateZ(20px); }
        
        /* Live Dynamic Coordinates Laser Mirroring */
        ...
        .gallery-caption {
          ...
          display: flex; flex-direction: column; justify-content: flex-end; padding: 40px; opacity: 0; transform: translateY(30px) translateZ(30px); transition: all 0.65s cubic-bezier(.16,1,.3,1); z-index: 3;
        }
        ...
        .gallery-zoom-icon {
          ...
          opacity: 0; backdrop-filter: blur(10px); transform: scale(0.6) translateZ(40px); transition: all 0.6s cubic-bezier(.16,1,.3,1); z-index: 4;
        }
        ...
        /* Production Videos Matrix Metrics */
        ...
        .reel-icon { font-size: 2.4rem; filter: drop-shadow(0 0 20px rgba(212,175,55,0.5)); animation: theaterIconWobble 5s infinite ease-in-out; }
        ...
        .video-card {
          ...
          border: 1px solid rgba(255,255,255,0.015); box-shadow: 0 30px 70px rgba(0,0,0,0.6); transition: all 0.8s cubic-bezier(.16,1,.3,1);
          transform-style: preserve-3d;
        }
        .video-card::after {
          content: ''; position: absolute; top: 0; left: -160%; width: 70%; height: 100%;
          background: linear-gradient(90deg, transparent, rgba(255,255,255,0.18), transparent);
          transform: skewX(-30deg); transition: none; z-index: 2;
        }
        .video-card:hover::after { left: 260%; transition: all 1.2s cubic-bezier(0.16, 1, 0.3, 1); }
        .video-card:hover { border-color: rgba(212,175,55,0.75); box-shadow: 0 60px 110px rgba(0,0,0,0.9), 0 0 40px rgba(212,175,55,0.2); }
        
        .video-card img { width: 100%; height: 100%; object-fit: cover; transition: all 0.95s cubic-bezier(.16,1,.3,1); filter: brightness(0.55) contrast(1.08); }
        .video-card:hover img { transform: scale(1.08) translateZ(10px); filter: brightness(0.88); }
        
        .play-button {
          position: absolute; top: 50%; left: 50%; transform: translate(-50%, -50%) scale(0.85) translateZ(30px);
          ...
          transition: all 0.5s cubic-bezier(.16,1,.3,1); z-index: 3;
        }
        .video-card:hover .play-button { transform: translate(-50%, -50%) scale(1.15) translateZ(50px); box-shadow: 0 30px 70px rgba(212,175,55,0.8); background: linear-gradient(135deg, #ffe28a, #d4af37); }
        ...
        /* Ornamental Symmetrical Structures */
        ...
        .ornament .line::after { content: ''; position: absolute; inset: 0; background: inherit; filter: blur(4px); }
        ...
        /* Theatrical Deep Lightbox Interface Frameworks */
        .lightbox-overlay {
          position: fixed; inset: 0; z-index: 3000; background: rgba(1,2,3,0.995);
          display: flex; align-items: center; justify-content: center; padding: 40px 20px;
          backdrop-filter: blur(30px); animation: overlayBlurFadeIn .5s cubic-bezier(0.25, 1, 0.5, 1) forwards;
        }
        @keyframes overlayBlurFadeIn { from { opacity: 0; backdrop-filter: blur(0px); } to { opacity: 1; backdrop-filter: blur(30px); } }
        
        .lightbox-img {
          max-width: 95vw; max-height: 86vh; border-radius: 20px; border: 1px solid rgba(255,255,255,0.12);
          box-shadow: 0 70px 150px rgba(0,0,0,1); transform: scale(0.9); animation: modalZoomIn .55s cubic-bezier(0.16, 1, 0.3, 1) forwards;
        }
        ...
        .lightbox-close {
          ...
          backdrop-filter: blur(10px); transition: all .45s cubic-bezier(0.16, 1, 0.3, 1);
        }
        ...
        .video-lightbox-frame {
          ...
          transform: scale(0.9); animation: modalZoomIn .55s cubic-bezier(0.16, 1, 0.3, 1) forwards;
        }
      ...
      {/* Global Cinematic Canvas Engines & Structural Filters */}
      ...
      {/* Real-time Top Scroll Tracker Indicator Line */}
      <div style={{ position: "fixed", top: 0, left: 0, height: 2, width: `${scrollProgress}%`, background: "linear-gradient(90deg, #8e6f23, #d4af37, #ffe28a)", zIndex: 9999, transition: "width 0.1s linear" }} />

      {/* ===== LUXURY TOP HEADER NAVIGATION ARCHITECTURE ===== */}
      <nav className="nav-bar-pf" style={{ position: "fixed", top: 0, left: 0, right: 0, zIndex: 1000, padding: "26px 6%", display: "flex", alignItems: "center", justifyBetween: "space-between", justifyContent: "space-between", background: "rgba(3,4,6,0.8)", backdropFilter: "blur(50px)", borderBottom: "1px solid rgba(255,255,255,0.015)" }}>
        <a href="/" style={{ display: "flex", alignItems: "center", gap: 16, textDecoration: "none" }}>
          ...
          <span style={{ color: "#fff", fontSize: "0.92rem", fontWeight: 900, letterSpacing: 9, transition: "color 0.4s" }} onMouseEnter={(e) => e.currentTarget.style.color = "#d4af37"} onMouseLeave={(e) => e.currentTarget.style.color = "#fff"}>FOCAL LENGTH</span>
        </a>
        <a href="/" className="back-link">← Back Home</a>
      </nav>

      {/* ===== HERO SECTION CHRONICLES ===== */}
      ...
      {/* ===== DISCOVER SYSTEM NAVIGATOR FILTER BADGES ===== */}
      <div className={`fu d1 ${loaded ? "show" : ""}`} style={{ display: "flex", justifyContent: "center", gap: 18, flexWrap: "wrap", padding: "20px 6%", position: "relative", zIndex: 4 }}>
      ...
      {/* ===== STUDIO INTERIORS GRID CANVAS ===== */}
      {(filter === "all" || filter === "studio") && (
        <section className={`gallery-sec-pf fu d2 ${loaded ? "show" : ""}`} style={{ padding: "110px 6%", maxWidth: 1440, margin: "0 auto", position: "relative", zIndex: 3 }}>
          <div style={{ textAlign: "center", marginBottom: 85 }}>
          ...
          <div className="gallery-grid">
            {studioImages.map((img, i) => (
              <div 
                ...
                onMouseMove={(e) => handleCardMouseMove(e, `st-${i}`)} 
              ...
              >
                ...
                <div className="gallery-caption">
                  <div style={{ width: 30, height: 1, background: "#d4af37", marginBottom: 16, transform: hoveredCard === `st-${i}` ? "scaleX(1.8)" : "scaleX(1)", transformOrigin: "left", transition: "transform 0.5s ease" }} />
      ...
      {/* ===== CINEMATIC THEATRE SHOWREELS SYSTEM ===== */}
      ...
      {/* ===== FINE ART IMAGE LIGHTBOX SYSTEM ===== */}
      {lightboxImg && (
      ...
      {/* ===== THEATRICAL VIDEO LIGHTBOX FRAMEWORK ===== */}
      {activeVideo && (
